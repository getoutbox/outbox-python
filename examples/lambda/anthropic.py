# Agent using Outbox Lambda destination + Anthropic
...
async def register_destination() -> None:
    await outbox.destinations.create(
        destination_id="lambda-anthropic",
        display_name="Lambda Anthropic agent",
        target_type="lambda",
        target_config={
...
async def process_event(payload: dict) -> None:
    ...
    async with outbox.messages.typing_indicator(
        ...
        if not isinstance(text_block, anthropic.types.TextBlock):
            return
        reply_text = text_block.text
