...
_VALID_CHANNEL_CONFIG_TYPES: frozenset[str] = frozenset(ChannelConfigType.__args__)
...
class ConnectorsNamespace:
    def __init__(self, base_url: str, interceptors: Interceptors = (), *, _poll_interval: float = 2.0) -> None:
    ...
    async def create_managed(
        ...
        *,
        ...
        if op.response.type_url != f"type.googleapis.com/{ProtoConnector.DESCRIPTOR.full_name}":
            msg = "create_managed_connector: failed to unpack response"
            raise RuntimeError(msg)
        connector_proto.ParseFromString(op.response.value)
