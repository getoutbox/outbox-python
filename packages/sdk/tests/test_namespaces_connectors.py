...
def ns(mock_conn_client: AsyncMock, mock_ops_client: AsyncMock) -> ConnectorsNamespace:
    with (
        ...
        return ConnectorsNamespace("http://localhost:8080", _poll_interval=0)
...
@pytest.mark.asyncio
async def test_update_no_args_sends_empty_field_mask(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    resp = connector_pb2.UpdateConnectorResponse()
    resp.connector.CopyFrom(_make_connector())
    mock_conn_client.update_connector = AsyncMock(return_value=resp)
...
@pytest.mark.asyncio
async def test_create_invalid_channel_config_type_raises(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    with pytest.raises(ValueError, match="Invalid channel_config_type"):
        await ns.create(
            channel_config_type=cast("ChannelConfigType", "not_a_real_channel"),
            channel_config={"some_key": "some_value"},
        )


@pytest.mark.asyncio
async def test_update_invalid_channel_config_type_raises(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    with pytest.raises(ValueError, match="Invalid channel_config_type"):
        await ns.update(
            id_="conn-1",
            channel_config_type=cast("ChannelConfigType", "not_a_real_channel"),
...
@pytest.mark.asyncio
async def test_list_with_filter_and_order_by(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    resp = connector_pb2.ListConnectorsResponse()
    mock_conn_client.list_connectors = AsyncMock(return_value=resp)
...
async def test_create_managed_success(
    ...
    packed = AnyProto()
    packed.type_url = f"type.googleapis.com/{connector_proto.DESCRIPTOR.full_name}"
    packed.value = connector_proto.SerializeToString()
...
async def test_create_managed_unpack_failure(
    ...
    any_val = AnyProto()
    # Set wrong type URL so create_managed_connector raises "failed to unpack response"
    any_val.type_url = f"type.googleapis.com/{Operation.DESCRIPTOR.full_name}"
    any_val.value = Operation().SerializeToString()
...
async def test_create_managed_forwards_webhook_url_and_tags(
    ...
    any_val = AnyProto()
    any_val.type_url = f"type.googleapis.com/{connector_proto.DESCRIPTOR.full_name}"
    any_val.value = connector_proto.SerializeToString()
    op_proto.response.CopyFrom(any_val)
    mock_conn_client.create_managed_connector = AsyncMock(return_value=op_proto)
