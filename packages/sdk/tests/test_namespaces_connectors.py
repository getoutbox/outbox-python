...
from outbox_sdk._enums import ConnectorState
from outbox_sdk._types import CreateConnectorResult
from outbox_sdk.namespaces._connectors import ConnectorsNamespace, ListConnectorsResult


def _make_connector(
    name: str = "connectors/conn-1",
    state: connector_pb2.Connector.State = connector_pb2.Connector.State.STATE_ACTIVE,
) -> connector_pb2.Connector:
    c = connector_pb2.Connector()
    c.name = name
    c.account.name = "accounts/acc-1"
    c.account.contact_id = "contact-1"
    c.account.external_id = "ext-1"
    c.state = state
    return c
...
@pytest.fixture
def ns(mock_conn_client: AsyncMock) -> ConnectorsNamespace:
    with patch("outbox_sdk.namespaces._connectors.ConnectorServiceClient") as MockClient:
        MockClient.return_value = mock_conn_client
        return ConnectorsNamespace("http://localhost:8080")
...
async def test_create_success(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    ...
    result = await ns.create(
        channel_config_type="whatsapp",
        channel_config={"app_id": "app123", "waba_id": "waba456"},
    )
...
async def test_create_with_authorization_url(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    resp = connector_pb2.CreateConnectorResponse()
    resp.connector.CopyFrom(_make_connector(state=connector_pb2.Connector.State.STATE_AUTHORIZING))
...
async def test_create_builds_correct_proto(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    ...
    assert req.connector.whatsapp.app_id == "app123"
    assert req.connector.whatsapp.waba_id == "waba456"
...
async def test_create_empty_response_raises(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    ...
    with pytest.raises(RuntimeError, match="server returned empty connector"):
        await ns.create(channel_config_type="whatsapp", channel_config={"app_id": "x"})
...
async def test_update_channel_config(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    ...
    req = mock_conn_client.update_connector.call_args[0][0]
    assert req.connector.whatsapp.app_id == "new-app"
    assert "whatsapp" in list(req.update_mask.paths)
...
@pytest.mark.asyncio
async def test_reauthorize(ns: ConnectorsNamespace, mock_conn_client: AsyncMock) -> None:
    ...
    url = await ns.reauthorize("conn-1")

    assert url == "https://oauth.example.com/reauth?token=abc"
