...
@pytest.mark.asyncio
async def test_create_invalid_target_type_raises(ns: DestinationsNamespace, mock_dest_client: AsyncMock) -> None:
    with pytest.raises(ValueError, match="Invalid target_type"):
        await ns.create(
            target_type=cast("DestinationTargetType", "not_a_real_target"),
            target_config={"url": "https://example.com"},
        )


@pytest.mark.asyncio
async def test_update_invalid_target_type_raises(ns: DestinationsNamespace, mock_dest_client: AsyncMock) -> None:
    with pytest.raises(ValueError, match="Invalid target_type"):
        await ns.update(
            id_="dest-1",
            target_type=cast("DestinationTargetType", "not_a_real_target"),
...
async def test_listen_yields_events(ns: DestinationsNamespace, mock_dest_client: AsyncMock) -> None:
    ...
    async def _run() -> None:
        events.extend([event async for event in ns.listen("dest-1")])
...
@pytest.mark.asyncio
async def test_listen_passes_cursor_to_next_request(ns: DestinationsNamespace, mock_dest_client: AsyncMock) -> None:
...
@pytest.mark.asyncio
async def test_listen_empty_response_loops_back(ns: DestinationsNamespace, mock_dest_client: AsyncMock) -> None:
    call_count = 0
